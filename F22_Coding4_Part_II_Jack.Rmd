...
BW.onestep = function(x, para){
  ...
  for(l in 1:mx){
    for (i in 1:mz){
      ...
      # if x[t] is equal to l, add to the numberator sum
...
myViterbi = function(x, para){
  ...
  for(t in 2:T){
    ...
    for(j in 1:mz){
      if (max((delta[t-1,j] + log(A[j,]))) > j_max){
        j_max = max((delta[t-1,j] + log(A[j,]))) #NOT SURE IF MAX LOGIC IS RIGHT
        j_index = j
      }
    }
    delta[t,] = j_max + log.A[j_index, ] + log.B[, x[t]]
  }
